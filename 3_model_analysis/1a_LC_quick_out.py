...
def save_outputs(args,save_dir='/scratch/bmac87/',from_logits=False,from_pickle=False,is_calibrated=False):
    ...
    if from_logits==True:#in logit space
        if from_pickle==True:
            ...
            y_pred = np.zeros(y_pred_logits.shape)
            if is_calibrated==True:
                ...
                for t,t_key in enumerate(['t0','t1','t2','t3']):
                    ...
                    y_pred[:,t,:,:] = tf.nn.sigmoid(y_pred_logits[:,t,:,:]/T).numpy()
            else:
                T = 1
...
def generate_merlin_output_figures_with_cartopy(y_true,y_pred,save_dir):
...
if __name__=='__main__':
    ...
    #     os.makedirs(img_dir)
    # generate_merlin_output_figures_with_cartopy(y_true=y_dict['y_true'], y_pred=y_dict['y_pred'],save_dir=img_dir)
