...
def make_performance_diagram_axis(ax=None,figsize=(5,5),CSIBOOL=True,FBBOOL=True,csi_cmap='Greys_r'):
    ...
    if CSIBOOL:
        ...
        pm = ax.contourf(X,Y,csi_vals,levels=np.arange(0,1.1,0.1),cmap=csi_cmap,alpha=.5)
    ...
    if FBBOOL:
        ...
        plt.clabel(bias, inline=True, inline_spacing=FREQ_BIAS_PADDING,fmt=FREQ_BIAS_STRING_FORMAT, fontsize=10,colors='Black')
