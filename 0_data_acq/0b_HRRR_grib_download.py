...
def main():
    ...
    for mo in yrs_dict[yr]:
        ...
        for day in days:
            for hr in hrs:
                hrrr_dir = '/scratch/bmac87/HRRR/HRRR_f%s/%s%s/'%(f_hour[-2:],yr,mo)
                print(hrrr_dir)
                archive_dir = '/ourdisk/hpc/ai2es/datasets/HRRR/HRRR_f%s/%s%s/'%(f_hour[-2:],yr,mo)
                hrrr_fname = 'hrrr_%s%s%s%s_%s.grib'%(yr,mo,day,hr,f_hour)
                if os.path.isfile(archive_dir+hrrr_fname)==False:
                    print(hrrr_fname)
                    missing_url = 'https://noaa-hrrr-bdp-pds.s3.amazonaws.com/hrrr.%s%s%s/conus/hrrr.t%sz.wrfprsf%s.grib2'%(yr,mo,day,hr,f_hour[-2:])
